{
    ...
    See the file COPYING.FPC, included in this distribution,menu_compile_compilermessages,
    menu_key_compile_compilermessagesavr
...
*********************************************************************
    ...
    diese somit als IDE fuer Microcontroller verwendet werden kann !


    letzte Modifikationen 27.12.2017   by R. Seelig
...
const
  rshelpfilename : string = '/usr/share/cide/chelp.ide';
  infozeils      = 18;
  ...
  ('                                                 ',
   ...
   '  ihm    einfache   C - Programme  fuer  Mikro-  ',
   '  controller  erstellt  werden.  Hierfuer  legt  ',
   '  CIDE hat eine Konfigurationsdatei unter        ',
   '                                                 ',
...
var
  ...
  confarmdir    : string = '~/.config/cide/.asm_makes.conf';
...
var
  utfstr   : array[0..utfanz-1] of string =
            (chr(226)+chr(128)+chr(152),
             ...
             chr(194)+chr(171),
             chr(194)+chr(187),
             chr(195)+chr(164),					// ä / ae             
             chr(195)+chr(132),					// Ä / Ae             
             chr(195)+chr(182),					// ö / oe             
             chr(195)+chr(150),					// Ö / Oe             
             chr(195)+chr(188),					// ü / ae             
             chr(195)+chr(156),					// Ü / ae             
             chr(195)+chr(159),					// ß / ss             
             chr(9) );
...
{$endif WinClipSupported}
  MenuBar:=New(PAdvancedMenuBar, Init(R, NewMenu(
    ...
    NewSubMenu('~C~all Make', 0, NewMenu(              
      ...
      NewLine(
      NewItem('Progra~m~arguments', '', kbnokey, cmcmdlineargs, hcNoContext,
      NewItem('R~u~n (PC only)', 'ALT-R', kbAltr, cmrunprog, hcNoContext,
      NewLine(
      NewItem('Compiler me~s~sages','F12', kbf12, cmCompilerMessages, hcCompilerMessages,
      nil)))))))))),

    NewSubMenu('RS232-~M~onitor',0,NewMenu(
...
begin
  ...
  StatusLine:=New(PIDEStatusLine, Init(R,
      NewStatusDef(0, $EFFF,
        ...
        NewStatusKey('~F7~ M.Commander', kbF7, cmcallmc,
...
procedure TIDEApp.commandlineargs;
var
  Dlg     : PDialog;
  R       : TRect;
  Control : PView;
...
begin
  R.Assign(12, 1, 80, 10);
  New(Dlg, Init(R, 'Commandline arguments'));
  
  R.Assign(2, 3, 23, 4);
  Control := New(PStaticText, Init(R, 'Commandline arguments'));
  Dlg^.Insert(Control);
  ...
  Dlg^.Insert(Control);  
  dlg^.setdata(cmdlineargs);  
  c:= desktop^.execview(dlg);  
  dlg^.getdata(cmdlineargs);  
end;
...
{$endif Unix}
           ...
           { -- Tools menu -- }
             ...
             cmcmdlineargs   : commandlineargs;
             cmrunprog       : callrunprog;
...
begin
  ...
  cname:= getmainfile(ccompile);
  if (cname='') then exit;
  fsplit(cname,sourcedir,na,s2);  
//  s2:= copy(cname,length(cname)-1,2);
  if ((s2<> '.c') and (s2<> '.cpp')) then
  begin
    MessageBox(#3'Wrong file or filename'#13 +
               #3'File-Extension must be .c'#13 +
               #3'or .cpp ',
...
//  s2:= copy(cname,length(cname)-1,2);
  ...
  if (s2= '.c') then
    ...
    oname:= copy(cname,1,length(cname)-4);
  ...
  close(f);
  DoExecute(oname,cmdlineargs,'','','',exNormal);
end;

procedure TIDEApp.callrunprog;
var
  olddir     : string;
...
begin
  ...
  compilermessagewindow^.clearmessages;
  compilermessagewindow^.addmessage(0,'--------','',1,1);
  compilermessagewindow^.addmessage(0,'Running program stoped...','',1,1);
  DoCompilerMessages;
  chdir(olddir);
end;
