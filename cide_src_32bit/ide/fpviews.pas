...
{$endif NODEBUG}
  ...
  FPSwitch,FPSymbol,FPDebug,FPVars,FPUtils,FPCompil,mytokens,FPHelp,
...
{  FreePascalAsmPrefix : string[8] = 'ASM';
...
{$endif DebugUndo}
          ...
          cmOpenAtCursor :
            begin
              ...
              if Pos('.',S)<>0 then
                OpenFileName:=S else
              OpenFileName:=S+'.c'+ListSeparator+
                            S+'.h';
...
begin
  if Editor^.FileName<>'' then
    ...
    begin
      SetTitle('noname'+IntToStrZ(NonameCount,2)+'.c');
    end;
end;
    ...
    if (W<>nil) and (W^.HelpCtx=hcSourceWindow) then
      begin
        ...
        if (E<>'') or (not tryexts) then
          ...
          begin
            Found:=SName=UpcaseStr(N+'.h');
            if Found=false then
              Found:=SName=UpcaseStr(N+'.c');
          end;
      end;
...
(*	New(Dlg, Init(R, 'About'));
	...
	R.Assign(16, 2, 32, 3);
	Control := New(PStaticText, Init(R, 'AVRIDE for Linux'));
	Dlg^.Insert(Control);
	
	R.Assign(19, 12, 29, 14);
	Control := New(PButton, Init(R, 'O~K~', cmOK, bfDefault));
	Dlg^.Insert(Control);
	
	R.Assign(4, 3, 44, 4);
	Control := New(PStaticText, Init(R, 'Target MCU''s: tiny2313 / mega8 / meg'+
	     'axx8'));
	Dlg^.Insert(Control);
	
	R.Assign(11, 6, 36, 7);
	Control := New(PStaticText, Init(R, 'Version 0.0.5 06/01/2015'));
	Dlg^.Insert(Control);
	
	R.Assign(13, 7, 34, 8);
	Control := New(PStaticText, Init(R, '(c) 2015 by R. Seelig'));
	Dlg^.Insert(Control);
	
	R.Assign(2, 9, 46, 11);
	Control := New(PStaticText, Init(R, '     AVRIDE based on the IDE of FPC b'+
	     'y'^M+
	     'B‚rczi G bor, Pierre Muller and Peter Vreman'));
	Dlg^.Insert(Control);
	
	R.Assign(6, 4, 39, 5);
	Control := New(PStaticText, Init(R, 'The beginner friendly AVR codeing'));
	Dlg^.Insert(Control);
    c2= execdesktop^.insert(dlg); *)
...
{$ifdef makeavride}  
  Insert(New(PStaticText, Init(R2, ^C' IDE for AVR, MCS51 & PC-Console')));
  R2.Move(0,1);
  Insert(New(PStaticText, Init(R2, ^C' Targets: AVR / AT89S52 / i386')));
  R2.Move(0,1);
  Insert(New(PStaticText, Init(R2, ^C'  Beginners friendly codeing')));	       
  R2.Move(0,2);
  Insert(New(PStaticText, Init(R2, ^C' Version 0.5.4 20/10/2015')));
  R2.Move(0,1);
  Insert(New(PStaticText, Init(R2, ^C' 2014 .. 2015 by R. Seelig')));
  R2.Move(0,2);
  Insert(New(PStaticText, Init(R2, ^C'     Linux IDE based on the IDE of FPC by')));
  R2.Move(0,1);
  Insert(New(PStaticText, Init(R2, ^C'B‚rczi G bor, Pierre Muller and Peter Vreman')));
{$endif makeavride}  

{$ifdef makegccide}  
  Insert(New(PStaticText, Init(R2, ^C'GCCIDE for Linux')));
  R2.Move(0,1);
  Insert(New(PStaticText, Init(R2, ^C'Target MCU''s: i386')));
  R2.Move(0,1);
  Insert(New(PStaticText, Init(R2, ^C'The beginner friendly GCC codeing')));	       
  R2.Move(0,2);
  Insert(New(PStaticText, Init(R2, ^C'Version 0.0.5 12/01/2015')));
  R2.Move(0,1);
  Insert(New(PStaticText, Init(R2, ^C'(c) 2015 by R. Seelig')));
  R2.Move(0,2);
  Insert(New(PStaticText, Init(R2, ^C'     GCCIDE based on the IDE of FPC by')));
  R2.Move(0,1);
  Insert(New(PStaticText, Init(R2, ^C'B‚rczi G bor, Pierre Muller and Peter Vreman')));
{$endif makegccide}    
  
(*  Insert(New(PStaticText, Init(R2, ^C'AVRIDE')));
  R2.Move(0,1);
